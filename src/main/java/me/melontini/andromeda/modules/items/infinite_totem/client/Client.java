...
public class Client {
    ...
    public static void init() {
        ClientPlayNetworking.registerGlobalReceiver(UsedCustomTotemPayload.ID, (payload, context) -> context.client().execute(() -> {
            var world = MakeSure.notNull(context.client().world, "client.world");
            Entity entity = world.getEntityLookup().get(payload.uuid());

            context.client().particleManager.addEmitter(MakeSure.notNull(entity, "(Andromeda) Client received invalid entity ID"), payload.particle(), 30);
            world.playSound(entity.getX(), entity.getY(), entity.getZ(), SoundEvents.ITEM_TOTEM_USE, entity.getSoundCategory(), 1.0F, 1.0F, false);
            if (entity == context.player()) context.client().gameRenderer.showFloatingItem(payload.stack());
        }));

        ClientPlayNetworking.registerGlobalReceiver(NotifyClientPayload.ID, (payload, context) -> context.client().execute(() -> {
            ItemEntity entity = (ItemEntity) MakeSure.notNull(context.client().world, "client.world").getEntityLookup().get(payload.entity());
