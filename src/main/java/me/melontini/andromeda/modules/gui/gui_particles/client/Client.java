...
public class Client {
    private static final Lazy<GuiParticles> module = Lazy.of(() -> () -> ModuleManager.quick(GuiParticles.class));

    public static void init() {
        ScreenEvents.BEFORE_INIT.register((client, screen1, scaledWidth, scaledHeight) -> {
            if (screen1 instanceof AbstractFurnaceScreen<?> abstractFurnaceScreen && module.get().config().furnaceScreenParticles) {
