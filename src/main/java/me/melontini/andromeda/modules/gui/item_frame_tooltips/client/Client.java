...
public class Client {
    ...
    private static void inGameTooltips() {
        HudRenderCallback.EVENT.register((matrices, delta) -> {
            if (MinecraftClient.getInstance().currentScreen == null) {
                ...
                if (action != null) {
                    renderFromComponents(client, matrices, action.get());
                }
            }
        });
    ...
    private static void renderFromComponents(MinecraftClient client, MatrixStack matrices, List<TooltipComponent> components) {
        ...
        DrawUtil.renderTooltipFromComponents(matrices, components, ((client.getWindow().getScaledWidth() / 2f) - (flow * 15)) + 15, ((client.getWindow().getScaledHeight() - j) / 2f) + 12);
