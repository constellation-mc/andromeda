      ...
      if (blockState.getRenderType() != BlockRenderType.INVISIBLE) {
        ...
        if (!MathHelper.approximatelyEquals(k, 0.0F)) {
          matrixStack.multiply(new Quaternionf()
              .setAngleAxis(
                  boatEntity.interpolateBubbleWobble(g) * (float) (Math.PI / 180.0),
                  1.0F,
                  0.0F,
                  1.0F));
        }
        ...
        Pair<Identifier, CompositeEntityModel<BoatEntity>> pair =
            this.texturesAndModels.get(boatEntity.getVariant());
        matrixStack.scale(0.8F, 0.8F, 0.8F);
        matrixStack.translate(
            0.5,
            pair.getSecond() instanceof RaftEntityModel ? 0.7 : 0.25,
            pair.getSecond() instanceof RaftEntityModel ? 1.06 : 1);
        // :sob:
        matrixStack.multiply(RotationAxis.POSITIVE_Y.rotationDegrees(-180));
        MinecraftClient.getInstance()
            .getBlockRenderManager()
            .renderBlockAsEntity(
                blockState, matrixStack, vertexConsumerProvider, i, OverlayTexture.DEFAULT_UV);
        matrixStack.pop();
      }
  }
}
