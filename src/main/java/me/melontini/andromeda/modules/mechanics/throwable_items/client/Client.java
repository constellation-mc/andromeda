...
public final class Client {
  ...
  public static void init(ThrowableItems.ClientConfig config) {
    ...
    ClientPlayNetworking.registerGlobalReceiver(
        ItemBehaviorsPayload.ID, (payload, context) -> context.client().execute(() -> {
            showTooltip.clear();
    ...
    ClientPlayNetworking.registerGlobalReceiver(
        FlyingStackLandedPayload.ID, (payload, context) -> context.client().execute(() -> {
    ...
    ClientPlayNetworking.registerGlobalReceiver(
        ColoredStackLandedPayload.ID, (payload, context) -> context.client().execute(() -> {
            int a = context.client().getWindow().getScaledWidth();
            ScreenParticleHelper.addParticle(new DyeParticle(MathUtil.nextDouble(a / 2d - (a / 3d), a / 2d + a / 3d), context.client().getWindow().getScaledHeight() / 2d,
                0,
                0,
                payload.dye()));
        }));
