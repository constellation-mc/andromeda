...
abstract class SpikeCacheMixin {
    ...
    private int andromeda$modifySpikeSize(int size) {
        if (Andromeda.getConfig(ModuleManager.quick(DragonFight.class)).c.shorterSpikes) return 72;
        return size;
    }
    ...
    private int andromeda$modifySpikeHeight(int size) {
        if (Andromeda.getConfig(ModuleManager.quick(DragonFight.class)).c.shorterSpikes) return 2;
        return size;
    }
}
