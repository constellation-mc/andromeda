        ...
        (stack, fie, world, user, hitResult) -> {
          if (hitResult.getType() == HitResult.Type.BLOCK) {
            ...
            Items.BONE_MEAL.useOnBlock(new ItemUsageContext(
                world,
                user instanceof PlayerEntity player ? player : null,
                Hand.MAIN_HAND,
                stack,
                result));
          }
    ...
    ItemBehaviorManager.register(
        (stack, fie, world, user, hitResult) -> {
          ...
          world.playSound(
              ...
              (float) (fie.getVelocity().normalize().length() * 1.5),
              1,
        ...
        (stack, fie, world, user, hitResult) -> {
          if (hitResult.getType() == HitResult.Type.BLOCK) {
            ...
            if (blockState.getBlock() instanceof TntBlock) {
              ...
              if (world
                  .getBlockState(blockPos = blockPos.offset(result.getSide()))
                  .isAir()) {
            ...
            if (entity instanceof LivingEntity livingEntity)
              livingEntity.takeKnockback(
                  0.4, -fie.getVelocity().getX(), -fie.getVelocity().getZ());
