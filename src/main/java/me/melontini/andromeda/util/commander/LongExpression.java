...
public interface LongExpression extends ToLongFunction<LootContext> {
    ...
    Codec<LongExpression> CODEC = ExtraCodecs.either(Codec.LONG, Codec.STRING).comapFlatMap((either) -> either.map(b -> DataResult.success(constant(b)), s -> Expression.parse(s).map(LongExpression::of)), LongExpression::toSource);
