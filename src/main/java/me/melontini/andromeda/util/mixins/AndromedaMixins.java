...
public class AndromedaMixins {
  ...
  private static final Map<String, Predicate<ClassNode>> SPECIAL_PREDICATES =
      ImmutableMap.<String, Predicate<ClassNode>>builder()
          .put(
              "common.mixin.util.DebugTrackerMixin",
              node -> Debug.Keys.DISPLAY_TRACKED_VALUES.isPresent())
          .build();
  ...
  public static boolean checkNode(ClassNode n) {
    ...
    var predicate = SPECIAL_PREDICATES.get(
        n.name.replace("/", ".").substring("me.melontini.andromeda.".length()));
  ...
  private static void verifyMixin(ClassNode mixinNode, String mixinClassName) {
    ...
    if ((mixinNode.access & Modifier.PUBLIC) == Modifier.PUBLIC) {
      builder.complaint("Invalid class modifier '%s'! remove 'public'"
          .formatted(Modifier.toString(mixinNode.access & ~Modifier.SYNCHRONIZED)));
    }
    if ((mixinNode.access & Modifier.ABSTRACT) != Modifier.ABSTRACT) {
      builder.complaint("Invalid class modifier '%s'! add 'abstract'"
          .formatted(Modifier.toString(mixinNode.access & ~Modifier.SYNCHRONIZED)));
    }
