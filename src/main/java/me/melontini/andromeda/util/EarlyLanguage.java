    ...
    for (Path path : list) {
      consume(
          ...
          e -> {
            ...
            var relative =
                gd.getFileSystem().equals(path.getFileSystem()) ? gd.relativize(path) : path;
            LOGGER.error("Failed to load {}! Deleting...", relative, e);
            consume(
                () -> Files.deleteIfExists(path),
                io -> LOGGER.error("Failed to delete {}!", relative, io));
          });
    }
    return Collections.unmodifiableMap(Maps.transformValues(
        map, input -> TOKEN_PATTERN.matcher(input.getAsString()).replaceAll("%$1s")));
  }
