...
public class CommonValues {
  ...
  private static final Supplier<ModContainer> MOD_CONTAINER =
      Memoize.supplier(() -> FabricLoader.getInstance().getModContainer(MODID).orElseThrow());
  private static final Supplier<String> MOD_VERSION =
      Memoize.supplier(() -> mod().getMetadata().getVersion().getFriendlyString());
  ...
  private static final Path CONFIG_PATH =
      FabricLoader.getInstance().getConfigDir().resolve("andromeda/mod.json");

  private static final Supplier<Platform> PLATFORM =
      Memoize.supplier(CommonValues::resolvePlatform);
  ...
  private static boolean checkUpdate() {
    ...
    if (holder.hasData("last_version")) {
      Version version =
          Exceptions.supply(() -> Version.parse(holder.getData("last_version").getAsString()));
      if (mod().getMetadata().getVersion().compareTo(version) != 0) {
        ...
        holder.modifyAndSave(() ->
            holder.putData("last_version", mod().getMetadata().getVersion().getFriendlyString()));
        return true;
      }
    } else {
      holder.modifyAndSave(() ->
  ...
  private static Platform resolvePlatform() {
    ...
    if (FabricLoader.getInstance().isModLoaded(Platform.QUILT.modId)) {
      String sn =
          MixinService.getService().getName().replaceAll("^Knot|^Launchwrapper|^ModLauncher|/", "");
  ...
  private static boolean isConnector() {
    if (FabricLoader.getInstance().isModLoaded(Platform.CONNECTOR.modId)) {
      try {
        ...
        Class.forName(
            "dev.su5ed.sinytra.connector.mod.ConnectorMod",
            false,
            CommonValues.class.getClassLoader());
        return true;
